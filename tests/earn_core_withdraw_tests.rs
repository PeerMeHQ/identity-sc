...
#[test]
fn it_partially_withdraws_core_stake_tokens() {
    let mut setup = setup::setup_contract(identity::contract_obj);
    let user_address = setup.user_address.clone();
    ...
    setup
        .blockchain
        .execute_tx(&user_address, &setup.contract, &rust_biguint!(0), |sc| {
            sc.withdraw_from_earn_endpoint(managed_token_id!(EARN_STAKE_CORE_TOKEN_ID), managed_biguint!(200));

            assert_eq!(sc.core_stake(&managed_address!(&user_address)).get(), managed_biguint!(300));
            assert_eq!(sc.core_stake_total().get(), managed_biguint!(300));
        })
        .assert_ok();
}
