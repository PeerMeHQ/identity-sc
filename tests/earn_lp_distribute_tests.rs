...
#[test]
fn it_distributes_to_lp_stakers() {
    ...
    setup
        .blockchain
        .execute_esdt_transfer(&user_address, &setup.contract, EARN_STAKE_LP_TOKEN_ID, 0, &rust_biguint!(stake_total), |sc| {
    ...
    setup
        ...
        .execute_esdt_transfer(&user_address, &setup.contract, CORE_TOKEN_ID, 0, &rust_biguint!(500), |sc| {
            ...
            sc.distribute_to_earn_lp_endpoint();

            assert_eq!(sc.lp_reward_per_token().get(), expected_rtp);
        })
        .assert_ok();
    ...
    setup
        .blockchain
        .execute_esdt_transfer(&user_address, &setup.contract, CORE_TOKEN_ID, 0, &rust_biguint!(500), |sc| {
            let expected_rtp = sc.lp_reward_per_token().get() + managed_biguint!(500) * managed_biguint!(10).pow(18) / stake_total;
